...
### Text Organization and Formatting - Phase 2 ###
...
wordstoremove <- c("afd", "germani", "german", "said", "year", "link", "post", 
                   ...
                   "write", "wrote", "tell", "just", "sinc", "since")
...
matrix(data = terms(guardian.lda, 15), nrow = 15, ncol = 4, byrow = FALSE)
...
# Create tf_idf-weighted dfm 
# (The Tf-idf is the frequency of a term adjusted for how rarely it is used)
phase2.guardian.tfidf <- dfm_tfidf(phase2.guardian.dfm)

# Select from main dfm using its top features
phase2.guardian.dfm <- dfm_keep(phase2.guardian.dfm, names(topfeatures(phase2.guardian.tfidf, n = 1000)))
...
# Convert 'quanteda dfm' to 'tm dtm' #better to run the topic models
phase2.guardian.dtm <- convert(phase2.guardian.dfm, to = "topicmodels")

# Using Latent Dirichlet Allocation (LDA) to run the topic model (Choice of 4 clusters)
phase2.guardian.lda <- LDA(phase2.guardian.dtm, k = 2)

# review terms by topic
terms(phase2.guardian.lda, 20)
matrix(data = terms(phase2.guardian.lda, 20), nrow = 20, ncol = 2, byrow = FALSE)
...
# Create tf_idf-weighted dfm 
# (The Tf-idf is the frequency of a term adjusted for how rarely it is used)
phase3.guardian.tfidf <- dfm_tfidf(phase3.guardian.dfm)

# Select from main dfm using its top features
phase3.guardian.dfm <- dfm_keep(phase3.guardian.dfm, names(topfeatures(phase3.guardian.tfidf, n = 1000)))


## Run the topic models ## 


# Goal: Want to observe the words appearing in the topic models, where I have 2 randomized clusters

# Convert 'quanteda dfm' to 'tm dtm' #better to run the topic models
phase3.guardian.dtm <- convert(phase3.guardian.dfm, to = "topicmodels")

# Using Latent Dirichlet Allocation (LDA) to run the topic model (Choice of 2 clusters)
phase3.guardian.lda <- LDA(phase3.guardian.dtm, k = 2)

# review terms by topic
terms(phase3.guardian.lda, 20)
matrix(data = terms(phase3.guardian.lda, 20), nrow = 20, ncol = 2, byrow = FALSE)
...
# Create tf_idf-weighted dfm 
# (The Tf-idf is the frequency of a term adjusted for how rarely it is used)
phase4.guardian.tfidf <- dfm_tfidf(phase4.guardian.dfm)

# Select from main dfm using its top features
phase4.guardian.dfm <- dfm_keep(phase4.guardian.dfm, names(topfeatures(phase4.guardian.tfidf, n = 1000)))
...
# Convert 'quanteda dfm' to 'tm dtm' #better to run the topic models
phase4.guardian.dtm <- convert(phase4.guardian.dfm, to = "topicmodels")

# Using Latent Dirichlet Allocation (LDA) to run the topic model (Choice of 2 clusters)
phase4.guardian.lda <- LDA(phase4.guardian.dtm, k = 2)

# review terms by topic
terms(phase4.guardian.lda, 20)
matrix(data = terms(phase4.guardian.lda, 20), nrow = 20, ncol = 2, byrow = FALSE)
